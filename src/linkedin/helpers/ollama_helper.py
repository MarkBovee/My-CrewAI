...
class LLMHelper:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # Memory optimization settings for 12GB GPU - maximum utilization
        self.memory_optimization = {
            'auto_unload': True,        # Automatically unload models when not needed
            'max_context_length': 16384, # Increased to 16,384 for MAXIMUM GPU utilization
    ...
    def get_optimal_context_length(self, model_name: str) -> int:
        ...
        """
        # Model size-based context optimization for maximum GPU utilization
        context_map = {
            # Small models (1.7B and below) - max out context for best performance
            'qwen2.5:0.5b': 16384,    # Maximum context for small models
            'qwen2.5:1.5b': 16384,    # Maximum context for small models
            'qwen2.5:3b': 16384,      # Maximum context for 3B models
            'qwen3:1.7b': 16384,      # Increased to 16,384 for maximum utilization
            # Medium models (7B range) - maximum contexts for best performance
            'openhermes:v2.5': 16384, # Increased to 16,384 for maximum utilization
            'llama3.2:3b': 16384,     # Increased to 16,384 for maximum utilization
            'llama3.1:7b': 12288,     # Conservative for 7B models
            # Larger models - moderate settings
            'llama3.1:13b': 8192,     # Conservative for large models
            'llama3.1:70b': 4096,     # Very conservative for very large models
        }
        
        # Get context length for specific model, with fallback to maximum default
        return context_map.get(model_name, 16384)  # Increased default to 16,384
